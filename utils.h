...
Int extract(std::vector<char>& data) {
  static_assert(sizeof(Int) == -1,
                "Default instantiation of this template is an error.");
}
...
inline std::pair<std::string, std::string> split(std::string str,
...
inline std::vector<std::string> read_resolv_conf(
    const std::string& path = "/etc/resolv.conf") {
